...
class FollowingService {
    ...
    func getFollowingList(by userID: Int, completion: @escaping (Result<[ProfileModel], Error>) -> ()) {
        requestService.request(InstagramEndPoint.followingList(userID: userID), interceptor: interceptor, serializationType: .JSON) { data, response, error in
            if let error = error {
